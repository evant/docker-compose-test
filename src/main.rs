...
fn run() -> Result<i32> {
    ...
    if !args.service_names.is_empty() {
        ...
        write_service_overlay(&mut overlay, &mut other_services, &args.service_names, &test_services)?;

        args.files.push(os_to_str_or_err(overlay.path().as_os_str())?);
    }

    up(&args)?;

    let code = wait_and_log(&args, &wait_services, &other_services)
        .and_then(|c| c.ok_or(Error::Custom("failed to get error code".to_owned())));
...
fn parse_args(mut args: Arguments) -> Result<Args> {
    let project_name = args.opt_value_from_str(["-p", "--project-name"])?
        .map(|p| Ok(p))
    ...
    let service_names = args.finish().into_iter()
    ...
    Ok(Args { project_name, verbose, files, service_names, docker, docker_compose })
}

fn current_dir_name() -> Result<String> {
    let dir = std::env::current_dir()?;
    let name = dir.file_name().ok_or(Error::Custom("failed to get file name on current dir".to_owned()))?;
    os_to_str_or_err(name)
}

fn os_to_str_or_err(s: &OsStr) -> Result<String> {
    Ok(s.to_str().ok_or(Error::Custom("invalid name".to_owned()))?.to_owned())
}

fn services_to_wait(service_names: &[String], services: &[(&str, Yaml)]) -> Result<Vec<String>> {
    if service_names.is_empty() {
        let result: Vec<String> = services.iter()
...
fn write_service_overlay(file: &mut NamedTempFile, other_services: &mut HashSet<String>, service_names: &[String], services: &[(&str, Yaml)]) -> Result<()> {
...
fn wait_and_log(args: &Args, wait_services: &[String], other_services: &HashSet<String>) -> Result<Option<i32>> {
...
fn wait(args: &Args, service: &str) -> Result<Option<i32>> {
    let name = service_name(args, service);
    Ok(Command::new(&args.docker).arg("wait").arg(&name).spawn()?.wait()?.code())
}

fn log(args: &Args, service: &str) -> Result<()> {
    let name = service_name(args, service);
    Command::new(&args.docker).arg("logs").arg(&name).spawn()?.wait()?;
    Ok(())
}
    ...
    if let Yaml::Mapping(entries) = contents {
        ...
        if let Some(services) = services {
            if let Yaml::Mapping(services) = services.value {
                let result: Vec<_> = services.into_iter().map(|entry| {
